...
async function loadCommentsCount() {
  try {
    const url = "https://script.google.com/macros/s/AKfycbxiZhDqPpPTAV4En1UHysdFFljXNpdpi65WEGQQkHCuTjRLjHVdic60duPHmorRrTq4OQ/exec?resource=comments&action=count";
...
function normalize(value, min, max, higherIsBetter) {
  ...
  if (min == null || max == null || Number.isNaN(min) || Number.isNaN(max) || max === min) return 0.5;
...
function filterResortsByRadius(resorts, origin, maxKm, originKey) {
  return resorts
    ...
    .map((r) => {
      ...
      const distanceKm = (typeof roadKm === "number")
        ? roadKm
...
function computeRanges(rows) {
  ...
  return {
    ...
    distMin: dist.min, distMax: dist.max,
    kmMin: km.min, kmMax: km.max,
    openKmMin: openKm.min, openKmMax: openKm.max,
    vibeMin: vibe.min, vibeMax: vibe.max,
    apresMin: apres.min, apresMax: apres.max,
    hfMin: hf.min, hfMax: hf.max,
...
function renderRows(rows) {
  ...
  const bestScore = Math.max(
    ...rows.map(r => r.score).filter(v => typeof v === "number")
  );
  ...
  for (const r of rows) {
    ...
    tr.appendChild(tdMetric(`${r.distanceKm != null ? r.distanceKm.toFixed(0) : "—"} km`, r.distanceKm, bw.distanceKm));
    tr.appendChild(tdMetric(r.kmTotal != null ? String(r.kmTotal) : "—", r.kmTotal, bw.kmTotal));
    tr.appendChild(tdMetric(
      r.openKm != null ? String(r.openKm) : "—",
      r.openKm,
      bw.openKm
    ));
    tr.appendChild(tdMetric(r.vibeScore != null ? String(r.vibeScore) : "—", r.vibeScore, bw.vibeScore));
    tr.appendChild(tdMetric(r.apresScore != null ? String(r.apresScore) : "—", r.apresScore, bw.apresScore));
    ...
    tr.appendChild(tdRightMetric(r.rentalPerPerson == null ? "—" : euro(r.rentalPerPerson), r.rentalPerPerson, bw.rentalPerPerson));
    tr.appendChild(tdRightMetric(r.hotelForfaitPerPerson == null ? "—" : euro(r.hotelForfaitPerPerson), r.hotelForfaitPerPerson, bw.hotelForfaitPerPerson));
    tr.appendChild(tdRightMetric(euro(r.totalPerPerson), r.totalPerPerson, bw.totalPerPerson));
    tr.appendChild(tdRightMetric(r.score == null ? "—" : `${round2(r.score).toFixed(1)}`, r.score, bw.score));
...
document.addEventListener("keydown", (e) => {
  ...
  $("commentsView")?.classList.add("hidden");
    ...
    $("openCommentsBtn")?.addEventListener("click", () => {
      ...
      window.openCommentsModalForResort?.("global", "Selecciona una estación (haz click en su nombre)");
    });
    ...
    $("topScoreBtn")?.addEventListener("click", () => { setActiveSeg("topScoreBtn"); showTopBy("score", "desc"); });
