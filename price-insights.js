...
window.openPriceInsights = async () => {
  ...
  try {
      ...
      for (let i = 0; i < series.length; i++) {
        ...
        if (prev && curr.cheapest != null && prev.cheapest != null) {
          ...
          if (diff < -0.00001) {
            ...
            const pct =
              prev.cheapest === 0 ? null : ((prev.cheapest - curr.cheapest) / prev.cheapest) * 100;
    ...
    renderRankCard(
      '🔴 Más volátiles (mayor volatilidad)',
      topVolatile,
      (it) => `Δ ${euroNum(it.m)}`
    );
    ...
    if (bestByCheapest)
      chip(`Mejor día (cheapest): ${bestByCheapest.day} · ${euroNum(bestByCheapest.avgCheapest)}`);
    if (bestDrop) chip(`Más “baja” (cheapest): ${bestDrop.day} · ${bestDrop.dropRate.toFixed(0)}%`);

    if (mostVolatileResort)
      chip(`Más volátil: ${mostVolatileResort.rid} · Δ medio ${euroNum(mostVolatileResort.m)}`);

    // Meta
    meta.textContent =
      'Basado en cheapestUnit y top10AvgUnit. Prob. bajar = % de veces que baja vs día anterior (por resort). % bajada media = magnitud media cuando baja.';
